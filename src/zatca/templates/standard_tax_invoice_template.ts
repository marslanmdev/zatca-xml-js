 ...
 *  name="0211010": BR-KSA-06 starts with "02" Standard Tax Invoice. Also explains other positions.
    ...
    `<cac:AccountingCustomerParty>
       <cac:Party>
         ...
         <cac:PostalAddress>
           ...
           <cbc:BuildingNumber>${props.buyer?.building ?? ""}</cbc:BuildingNumber>
           <cbc:CityName>${props.buyer?.city ?? ""}</cbc:CityName>
           <cbc:PostalZone>${props.buyer?.postal_code ?? ""}</cbc:PostalZone>
           <cac:Country>
             <cbc:IdentificationCode>SA</cbc:IdentificationCode>
           </cac:Country>
         </cac:PostalAddress>
         <cac:PartyLegalEntity>
           <cbc:RegistrationName>${props.buyer?.name ?? ""}</cbc:RegistrationName>
         </cac:PartyLegalEntity>
       </cac:Party>
     </cac:AccountingCustomerParty>`
  );
